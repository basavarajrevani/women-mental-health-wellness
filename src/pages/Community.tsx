...
import { syncAdminDataToPublic, debugPublicData } from '../utils/dataSyncUtils';
import {
  MessageCircle,
  Users,
  Calendar,
...
const Community: React.FC = () => {
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      try {
        // Force sync admin data to public storage
        ...
        setSupportGroups(publicSupportGroups);
        setEvents(publicEvents);
        setIsInitialized(true);

        console.log('✅ Community page initialized with:', {
          supportGroups: publicSupportGroups.length,
    ...
    if (!isInitialized) {
      // Add a small delay to ensure contexts are ready
      const timeoutId = setTimeout(initializeData, 500);
      return () => clearTimeout(timeoutId);
    }
  }, [isInitialized]); // Run when isInitialized changes

  // Load public data on component mount and when tab changes
  useEffect(() => {
    const loadPublicData = () => {
      // Force sync admin data to public storage first
      console.log('🔄 Ensuring data sync...');
      syncAdminDataToPublic();

      // Then load the public data
      const publicSupportGroups = publicDataHelpers.getPublicSupportGroups();
      const publicEvents = publicDataHelpers.getPublicEvents();
      const allPublicData = publicDataHelpers.loadPublicData();

      setSupportGroups(publicSupportGroups);
      setEvents(publicEvents);

      console.log('📂 Loaded public support groups:', publicSupportGroups.length);
      console.log('📂 Loaded public events:', publicEvents.length);
      console.log('🔍 All public data:', allPublicData);
      console.log('🔍 Raw support groups:', allPublicData.supportGroups);
      console.log('🔍 Raw events:', allPublicData.events);
    };
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'wmh_shared_app_data' || e.key === 'wmh_public_data') {
    ...
    // Refresh data every 10 seconds to get latest admin changes
  ...
  const handleAddComment = (postId: string) => {
    ...
    const userId = authUser.id;
    const username = authUser.name || authUser.email.split('@')[0];
    const userAvatar = '👤'; // Default avatar
  ...
  // Debug function to manually sync data
  const handleDebugSync = () => {
    console.log('🔄 Manual debug sync triggered');
    const allPublicData = publicDataHelpers.loadPublicData();
    console.log('🔍 Debug - All public data:', allPublicData);

    const publicSupportGroups = publicDataHelpers.getPublicSupportGroups();
    const publicEvents = publicDataHelpers.getPublicEvents();

    console.log('🔍 Debug - Filtered support groups:', publicSupportGroups);
    console.log('🔍 Debug - Filtered events:', publicEvents);
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  const result = syncAdminDataToPublic();
                  console.log('Sync result:', result);
                  handleDebugSync(); // Refresh after sync
                }}

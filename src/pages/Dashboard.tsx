    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50">
      ...
      <main className="lg:ml-64 pt-16 lg:pt-0 p-4 sm:p-6 lg:p-8">
        <div className="max-w-7xl mx-auto">
          ...
          {/* Daily Activities & Quick Actions */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
              ...
              <div className="flex items-center mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {moodEntries.some(entry =>
                      new Date(entry.timestamp).toDateString() === new Date().toDateString()
                    ) ? 'Completed today ✓' : 'Track your mood today'}
                  </p>
                </div>
              </div>
              <p className="text-gray-600 mb-4">
                Take a moment to reflect on your feelings and track your emotional well-being.
              </p>
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="w-full bg-gradient-to-r from-green-500 to-emerald-500 text-white px-4 py-3 rounded-lg hover:shadow-lg transition-all font-medium"
                disabled={moodEntries.some(entry =>
                  new Date(entry.timestamp).toDateString() === new Date().toDateString()
                )}
              >
                {moodEntries.some(entry =>
                  new Date(entry.timestamp).toDateString() === new Date().toDateString()
                ) ? 'Completed Today' : 'Start Check-in'}
              </motion.button>
            </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow">
              ...
              <p className="text-gray-600 mb-4">
                {upcomingEvents.length > 0
                  ...
                  : 'We\'ll notify you when new group sessions are available.'
                }
              </p>
              {upcomingEvents.length > 0 && (
              ...
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="w-full bg-gradient-to-r from-blue-500 to-indigo-500 text-white px-4 py-3 rounded-lg hover:shadow-lg transition-all font-medium disabled:opacity-50"
                disabled={upcomingEvents.length === 0}
              >
                {upcomingEvents.length > 0 ? 'Join Session' : 'No Sessions Available'}
              </motion.button>
            </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow">
              <div className="flex items-center mb-4">
                ...
                <div>
                  <h3 className="text-xl font-semibold text-gray-800">Mindfulness</h3>
                  <p className="text-sm text-gray-500">5-minute session</p>
                </div>
              </div>
              <p className="text-gray-600 mb-4">
                Take a quick mindfulness break to center yourself and reduce stress.
              </p>
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="w-full bg-gradient-to-r from-purple-500 to-pink-500 text-white px-4 py-3 rounded-lg hover:shadow-lg transition-all font-medium"
              >
                Start Session
              </motion.button>
            </div>
